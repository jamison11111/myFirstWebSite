...
public class HttpUtil {
	 ...
	 * http get请求 返回输出流
	 * @param url 请求链接
	 * @param headers 请求头,将请求转发给DFS服务器
	 * @param response 响应，承载返回的输出流
	 */
	public static OutputStream get(String url,
		...
		//创建响应体的输出流对象，这个对象可以把程序中的数据写进响应体里
		OutputStream os = response.getOutputStream();
		int responseCode = con.getResponseCode();
		byte[] buffer = new byte[1024];
		if(responseCode >=200 && responseCode <300) {
			int i = bis.read(buffer);//通过输入流，把数据从服务器读到内存程序中的buffer数组里
			while (( i != -1)) {
				os.write(buffer,0,i);//响应体的输出流对象把程序中buffer里的数据读入(目的地为响应体的输出流对象)
				i = bis.read(buffer);
			}
			bis.close();
		}
		bis.close();
		con.disconnect();
		return os;//返回响应体的输出流对象，也即输出流被封装到了响应体里
	}
